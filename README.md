...
- [MinIO (файловое хранилище)](#minio-файловое-хранилище)
...
| `minio-service` | Объектное хранилище (S3-совместимое) для файлов вложений | `9000` (API), `9001` (Console) |
| `minio-setup` | Одноразовый сервис - создаёт bucket `dev-bucket` при первом запуске | - |
...
MinIO доступен по адресу `http://localhost:9000` для S3-совместимых запросов (используется `main-service` для загрузки/скачивания файлов).

### Автоматическое создание bucket

Bucket `dev-bucket` создаётся автоматически при первом запуске - через отдельный сервис `minio-setup` (аналогично `keycloak-seed-data` - использует `mc` (MinIO Client), дожидается готовности MinIO и выполняет `mc mb`).
...
Ожидается `Bucket created successfully` (или сообщение о том, что bucket уже существует, при повторном запуске).
